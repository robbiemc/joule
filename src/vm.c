...
#define REG(f, n)                                                             \
  ({                                                                          \
    ...
    luav _tmp = stack[n];                                                     \
    lv_istype(_tmp, LUPVALUE) ? lv_getupvalue(_tmp)->value : _tmp;            \
  })
#define SETREG(f, n, v)                       \
  ({                                          \
    assert((n) < STACK_SIZE(f));              \
    if (lv_istype(stack[n], LUPVALUE)) {      \
